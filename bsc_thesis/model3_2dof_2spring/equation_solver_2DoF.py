...
def symbolic_solver(m_data, l_data):
    ...
    # !!!!!!!!!
    x_2 = x + 3/4*l*sp.sin(phi)

    U = (1/2)*k_1*(r-x)**2+(1/2)*k_t*(phi)**2+ (1/2)*k_2*(r-x_2)**2
    ...
    ddx_num = lambdify((t, x, dx, phi, dphi, r, k_1, k_2, k_t), ddx_num_expr, 'numpy')
    ddphi_num = lambdify((t, x, dx, phi, dphi, r, k_1, k_2, k_t), ddphi_num_expr, 'numpy')
...
def num_solver(ddx_num, ddphi_num, gain_input, time_input, y0, k_1, k_2, k_t):
    ...
    def system(t, y):
        ...
        ddx_val = float(ddx_num(t, x_val, dx_val, phi_val, dphi_val, r, k_1, k_2, k_t))
        ddphi_val = float(ddphi_num(t, x_val, dx_val, phi_val, dphi_val, r, k_1, k_2, k_t))
...
def acceleration_substituted(ddx_num, ddphi_num, gain_input, time_input, y0,  k_1, k_2, k_t):
    x_values, dx_values, phi_values, dphi_values, _ = num_solver(ddx_num, ddphi_num, gain_input, time_input, y0, k_1, k_2, k_t)
    gain = gain_input
    ...
    # for i in range(1400):
        # i = int(i)
    ddx_subst = ddx_num(time_input, x_values, dx_values, phi_values, dphi_values, gain, k_1, k_2, k_t)
    ddphi_subst = ddphi_num(time_input, x_values, dx_values, phi_values, dphi_values, gain, k_1, k_2, k_t)
...
def calculate_acceleration_components_for_all_body_parts( ddx_num, ddphi_num, sled_disp, sim_time, y0, k_1, k_2, k_t, l ):

    x_computed, dx_computed, phi_computed, dphi_computed, t_computed = \
        num_solver( ddx_num, ddphi_num, sled_disp, sim_time, y0, k_1, k_2, k_t ) # displacements in mm, velocities in mm/s
    
    a_pelvis_x, ddphi_computed = \
        acceleration_substituted( ddx_num, ddphi_num, sled_disp, sim_time, y0, k_1, k_2, k_t )
