...
study_name_3dof = "3dof_k_inits"
max_num_of_trials = 120  # How many iterations optuna should go through, before it picks the best parameters.
...
output_csv_path = os.path.join(optuna_3dof_opt_main_dir, "trials_ch_" + study_name_3dof + ".csv")

# Results of each trial (including the best) will be stored in an sqlite database.
storage_name = os.path.join( f"sqlite:///{optuna_3dof_opt_main_dir}", f"{study_name_3dof}.db" )

study = optuna.create_study(study_name=study_name_3dof,
                            storage=storage_name,
                            direction='minimize',
                            load_if_exists=True,
                            )

study.optimize(lambda trial: lmfit_3dof.optuna_obj_combined(trial, ddx_num, ddphi_num, ddtheta_num, gain, simulation_time_vals, pelvis_acceleration_simu_x, chest_acceleration_simu_x, head_acceleration_simu_x, y0, R, l, channels_to_use),
